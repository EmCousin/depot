class LineItemsController < ApplicationController
  skip_before_action :authorize, only: [:create, :increment, :decrement, :destroy]
  include CurrentCart
  ...
  def create
    ...
    respond_to do |format|
      if @line_item.save
        ...
        session[:counter] = 0
        # With format.js, when create finishes handling the AJAX request, Rails will look for a
  ...
  # POST /line_items/decrement.json
  def decrement
    product = Product.find(params[:product_id])
